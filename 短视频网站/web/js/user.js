...
$(function () {
    $("#userName").blur(function () { // 当xxx失去焦点时
        ...
        if (userNameValue != "") {
            $.ajax({
                url: "http：//localhost/ajaxChcekUserName", // 要请求的服务器url
    ...
    $("#userPassword").blur(function () { // 当xxx失去焦点时
        ...
        if (userPasswordValue != "" && null != userNameValue) {
            $.ajax({
                url: "http：//localhost/ajaxChcekPass", // 要请求的服务器url
                // 这是一个对象，表示请求的参数，两个参数：method=ajax&val=xxx，服务器可以通过request.getParameter()来获取
                // data:{method:"ajaxTest",val:value},
                data: {
    ...
    $("#edPass").blur(function () { // 当xxx失去焦点时
        ...
        if (edPassValue != "" && null != edPassValue) {
            $.ajax({
                url: "http：//localhost/ajaxChcekUserPassByUserName", // 要请求的服务器url
                // 这是一个对象，表示请求的参数，两个参数：method=ajax&val=xxx，服务器可以通过request.getParameter()来获取
                // data:{method:"ajaxTest",val:value},
                data: {
                    username: userNameValue,
                    password: edPassValue
                }, // 这里的email对应表单中的name="email"，也是发送url中的email=value(GET方式)
                async: true, // 是否为异步请求
                cache: false, // 是否缓存结果
                type: "POST", // 请求方式为POST
                    ...
                    $("#reUserPassword").blur(function () { // 当xxx失去焦点时
                        ...
                        if (rePass != '') {
                            if (userPass != rePass) {
                                ...
                                var text = $("#confirRePassSpan").innerText;
                                alert(text);
                            } else {
...
function reCheck(form) {
    ...
    if (form.reUserPassword.value == '' || text == "密码不一致!") {
