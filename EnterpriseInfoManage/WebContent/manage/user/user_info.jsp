...
var enterpriseName="";
	...
	function loding(pageNum){
		$.ajax({
			async:false,
			type:"post",
			url:"http://"+test+"/EnterpriseInfoManage/userController/getAllUser?pageNum="+pageNum+"&pageSize="+pageSize,
			dataType:"json",
			success:function(data){
				var html ="";
				$.each(data,function(index,item){
					getEnterpriseName(item.enterprise_id);
					html +="<tr><td>"+item.user_name+"</td><td>"+item.user_state+"</td><td>"+item.user_pwd+"</td><td>"+enterpriseName+"</td><td><a href='manage/user/updUser.jsp?userId="+item.id+"&username="+username+"&userState="+userState+"'><input type='button' value='修改'/></a><input type='button'  value='删除' onclick='delUser("+item.id+")'/></td></tr>";
					enterpriseName="";
				})
				if(html==""){
	...
	function fenYe(){
       	$.ajax({
       		async:false,
       		type:"post",
       		url:"http://"+test+"/EnterpriseInfoManage/userController/getLen",
       		success:function(data){
       			len=data;	
       		}
       	})
	...
	function delUser(id){
		...
		$.ajax({
			type:"post",
			url:"http://"+test+"/EnterpriseInfoManage/userController/del?id="+id,
			dataType:"json",
			success:function(data){
				if(data>0){
					alert("删除成功");
...
<body>
	...
	<div class="model_list page_left">
		...
		<div class="model_list_text">
			<a href="manage/user/addUser.jsp?username=<%=request.getParameter("username")%>&userState=<%=request.getParameter("userState")%>"><div class="model_operation model_list_add">添加</div></a><hr/>
		</div>
	</div>
