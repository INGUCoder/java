	...
	function loading(){
		$.ajax({
			...
			success:function(data){
				$("#enterpriseZoningDivision").val(data.enterpriseZoningDivision);
				$("#enterpriseUnifiedSocialCreditCode").val(data.enterpriseUnifiedSocialCreditCode);
				$("#enterpriseOrganizationalCode").val(data.enterpriseOrganizationalCode);
				$("#enterpriseDetailedName").val(data.enterpriseDetailedName);
				$("#enterpriseLegalRepresentative").val(data.enterpriseLegalRepresentative);
				$("#enterpriseAddressStatus").val(data.enterpriseAddressStatus);
				$("#enterpriseDetailedAddress").val(data.enterpriseDetailedAddress);
				$("#enterpriseMobilePhone").val(data.enterpriseMobilePhone);
				$("#enterpriseBusinessActivities").val(data.enterpriseBusinessActivities);
				$("#enterpriseIndustryCode").val(data.enterpriseIndustryCode);
				$("#enterpriseRegistrationTypes").val(data.enterpriseRegistrationTypes);
				$("#enterpriseIsHaveBranchOffice").val(data.enterpriseIsHaveBranchOffice);
				$("#enterpriseArea").val(data.enterpriseArea);
				$("#_611").val(data._611);
				$("#_6111").val(data._6111);
				$("#_6112").val(data._6112);
				$("#_6113").val(data._6113);
				$("#_61144").val(data._61144);
				$("#_6115").val(data._6115);
				$("#_6116").val(data._6116);
				$("#_61177").val(data._61177);
				$("#enterpriseEnergyConsumption").val(data.enterpriseEnergyConsumption);
				$("#enterpriseInvestmentFixedAssets").val(data.enterpriseInvestmentFixedAssets);
				$("#enterpriseRegistrationType").val(data.enterpriseRegistrationType);
				$("#enterpriseOperationState").val(data.enterpriseOperationState);
				$("#enterpriseRegistrationStatus").val(data.enterpriseRegistrationStatus);
				$("#enterpriseSupplementaryItems").val(data.enterpriseSupplementaryItems);
	   		}
		})
	}
	
	
	function updEnterprise(){
	  	$.ajax({
       		async:false,
       		type:"post",
       		url:"http://"+test+"/EnterpriseInfoManage/enterpriseInfoController/updEnterprise",
       		dataType: "json",
       		data:{
       			enterpriseId:enterpriseId,
       		...
       		success:function(data){
       			if(data>0){
       				alert("修改成功");
       				location.href="http://"+test+"/EnterpriseInfoManage/manage/enterprise/enterprise_info.jsp";
       			}
       		}
       	})
...
<body>
	<div class="head">
		...
		<div class="head_model">
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<a href="manage/main.jsp?username=<%=request.getParameter("username")%>" style="color: white;">首页</a>
			<a href="manage/enterprise/enterprise_info.jsp?username=<%=request.getParameter("username")%>" class="model" style="color: white;background-color:#5B00AE ">企业信息管理</a>
			<a href="manage/user/user_info.jsp?username=<%=request.getParameter("username")%>" class="model" style="color: white;">系统角色管理</a>
		</div>
	</div>
	...
	<table>
		<thead>
			<tr class="head_text"  height="50px"><td colspan="2" style="padding-left: 130px">修改企业信息</td></tr><br/><br/>
		</thead>
		<tbody>		
